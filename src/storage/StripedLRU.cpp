...
bool StripedLRU::Put(const std::string &key, const std::string &value) {
    return _stripes[_stripes_hash(key) % _stripes_cnt]->Put(key, value);
}

bool StripedLRU::PutIfAbsent(const std::string &key, const std::string &value) {
    return _stripes[_stripes_hash(key) % _stripes_cnt]->PutIfAbsent(key, value);
}

bool StripedLRU::Set(const std::string &key, const std::string &value) {
    return _stripes[_stripes_hash(key) % _stripes_cnt]->Set(key, value);
}

bool StripedLRU::Delete(const std::string &key) {
    return _stripes[_stripes_hash(key) % _stripes_cnt]->Delete(key);
}

bool StripedLRU::Get(const std::string &key, std::string &value) {
    return _stripes[_stripes_hash(key) % _stripes_cnt]->Get(key, value);
}
